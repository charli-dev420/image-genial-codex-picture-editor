...
11. When the widget sends an exact handoff through `ui/message` (for example after “Lancer Image Gen natif”), treat it as the user's explicit request to continue in this same turn. The widget has not run Image Gen itself; Codex must execute the following steps.
12. Record `codex_imagegen_requested` or `waiting_for_codex_image_gen` only when the exact handoff has been sent and Codex is actually about to run or has just run the built-in Image Gen flow. Do not record those statuses at simple prompt export time.
13. If the edit target or references are local files, first make them visible to Codex with `view_image`, then call the built-in `image_gen` tool.
14. Move or copy the selected built-in Image Gen artifact into the workspace path specified by the context packet.
15. Register the candidate with `register_artifact_candidate`. Accept it with `resolve_artifact_candidate` only when `origin` is exactly `codex-image-gen`; this delegates to `save_generated_result`. Use `save_generated_result` directly only for compatibility with older flows.
16. Use `compare_version` for review diagnostics and `create_retry_request` when a version is rejected or frozen-region drift is detected.
17. Use `list_pending_generations`, `list_versions`, `commit_version`, or `reject_version` for final review decisions.
